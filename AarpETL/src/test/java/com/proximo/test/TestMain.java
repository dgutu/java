...
public class TestMain {
	...
	public static String builJsonURL(String since, String page, String limit, String key) {
		return ConfigurationManager.jsonbaseurl.concat("since=1422975728&page=").concat(page).concat("&limit=")
				.concat(limit).concat("&key=").concat(key);
	}
	...
	public static void test1() {
		...
		String url = builJsonURL("", String.valueOf(page), ConfigurationManager.jsonlimit,
	...
	public static void startLR_ETL_Process_Type() {
		try {
			DatabaseManager dm = new DatabaseManager(ConfigurationManager.jdbc_driver, ConfigurationManager.db_url,
					ConfigurationManager.database, ConfigurationManager.dbuser, ConfigurationManager.dbpassword,
					ConfigurationManager.getSince(), ConfigurationManager.batchsize, "");
			...
			String apiURL = ConfigurationManager.jsonbaseurl + "since=" + dm.getLastSinceUnixTimeStamp() + "&page="
					+ ConfigurationManager.jsonpagestart + "&limit=" + ConfigurationManager.jsonlimit + "&key="
					+ ConfigurationManager.jsonkey;
	...
	public static void undefinedJson() {
		...
		try {
			JsonParser parser = mapper.getJsonFactory()
					.createJsonParser(new File("/Users/dumitrugutu/Downloads/test_json.txt"));
	...
	public static void readJsonSprint2(DatabaseManager dm, String url, Long processID, String contentType) {
		...
		try {
			...
			ObjectMapper mapper = new ObjectMapper();
			// JsonParser parser = mapper.getJsonFactory().createJsonParser(new
			// File("/Users/dumitrugutu/Downloads/json_cms_challenge.txt"));
			JsonParser parser = mapper.getJsonFactory()
					.createJsonParser(new File("/Users/dumitrugutu/Downloads/aaa.json"));
			// JsonParser parser = mapper.getJsonFactory().createJsonParser(new
			// File("/Users/dumitrugutu/Downloads/activity_result/activity_results1.json"));
			// JsonParser parser = mapper.getJsonFactory().createJsonParser(new
			// File("/Users/dumitrugutu/Downloads/app_progress/app_progress1.json"));
			// JsonParser parser = mapper.getJsonFactory().createJsonParser(new
			// File("/Users/dumitrugutu/Downloads/Ecommerce_collections/ecommerce_purchase_orders_624.json"));
			// JsonParser parser = mapper.getJsonFactory().createJsonParser(new
			// File(url));
			// JsonParser parser = mapper.getJsonFactory().createJsonParser(new
			// URL("http://cms.lifereimagined.org/webservice/getcontent.json?content_type=challenge_supporting_material"));
			// JsonParser parser = mapper.getJsonFactory().createJsonParser(new
			// File("/Volumes/External Flash/users_tmp.json.txt"));
			// JsonParser parser = mapper.getJsonFactory().createJsonParser(new
			// File("/Users/dumitrugutu/Downloads/users_USER_OID.json"));
			...
			// ps = dm.getConnection().prepareStatement("{call
			// LoadUndefineJSON_v2(?,?,?,?,?,?,?,?,?,?,?)}");
			...
			while (!parser.isClosed()) {
				...
				while (parser.nextToken() != null) {
					...
					if (!node.toString().equals("null")) {
						...
						/*
						 * if ((step % Integer.valueOf(ConfigurationManager.
						 * historical_load_step)) == 0) {
				...
				/*
				 * dm.loadStg_Json_Structure(); dm.loadStagingHistorical();
				 * dm.refreshHistDW_v2(); dm.truncateStg_Json_Structure();
				 */
	...
	public static void parseObject(DatabaseManager dm, JsonNode node, int level, String array_indx, long parent_id,
			String parent_str) throws JsonProcessingException, IOException, SQLException {
		...
		while (iterator.hasNext()) {
			...
			// id++;
			// dm.loadUndefineJSON_v2(ps, ++id, key, temp.toString(), parent_id,
			// temp.asToken().toString(), numberOfTokens, 10L, "MongoDB_v2",
			// level, null, array_indx);
			parent_str = oldparent_str + "." + key;

			dm.loadUndefineJSON_v2(++id, key, temp.toString(), parent_id, temp.asToken().toString(), numberOfTokens,
					10L, "MongoDB_v2", level, parent_str, array_indx, temp.toString());
	...
	public static void parseArray(DatabaseManager dm, JsonNode node, int level, String array_indx, long parent_id,
			String parent_str) throws JsonProcessingException, IOException, SQLException {
	...
	/*
	 * public static void printJsonObject(DatabaseManager dm, JsonNode node,
	 * Long processID, String contentType) {
	 * 
	 * int index = 1;
	 * 
	 * //int index = 1; long parent_id = 0L;
	 * 
	 * if (flag_next_token) { parent_id = 0L; flag_next_token = false; } else {
	 * parent_id = dm.getMaxIdJsonUndefineStructure(); }
	 * 
	 * // object type JSON if (node.isObject()) {
	 * 
	 * Iterator<Map.Entry<String,JsonNode>> iterator=node.getFields();
	 * 
	 ...
	 * 
	 * String key = jsonNode.getKey(); JsonNode temp = jsonNode.getValue();
	 * 
	 * // insert into database if (temp.asToken() == JsonToken.START_OBJECT)
	 * dm.loadUndefineJSON(key, null, parent_id, temp.asToken().toString(),
	 * index, processID, contentType, null); else dm.loadUndefineJSON(key,
	 * temp.toString(), parent_id, temp.asToken().toString(), index, processID,
	 * contentType, null);
	 * 
	 * printJsonObject(dm, temp, processID, contentType); } } // array type JSON
	 * else if (node.isArray()) { for (int i=0;i<node.size();i++) { index++;
	 * 
	 * JsonNode child = node.get(i); printJsonObject(dm, child, processID,
	 * contentType); } } /*else { if (node.isValueNode()) {
	 * logger.debug("valueNode: " + node.asText()); } else {
	...
	public static void readJsonObjectNode(JsonNode node) {
		...
		if (node.isObject()) {
			...
			while (i.hasNext()) {
				...
				/*
				 * if (temp.isArray()) { System.out.print(key+"=");
				 * System.out.println(temp);
				 * 
				 * readJsonArrayNode(temp); parentId++; } else
				 */if (temp.isObject()) {
					System.out.println();
	...
	/*
	 * public static void testS3Amazon() { try { String awsPath =
	 * ConfigurationManager.aws_path; String awsAccessKey =
	 * ConfigurationManager.aws_access_key; String awsSecret =
	 * ConfigurationManager.aws_secret;
	 * 
	 * String testFilePath = "/Users/dumitrugutu/Downloads/"; String testFile =
	 * "export.log";
	 * 
	 * AWSCredentials awsCredentials = new BasicAWSCredentials(awsAccessKey,
	 * awsSecret); AmazonS3 s3Client = new AmazonS3Client(awsCredentials);
	 * 
	 ...
	 * putObjectRequest = new PutObjectRequest(awsPath, testFile, stream,
	 * objectMetadata); PutObjectResult result =
	 * s3Client.putObject(putObjectRequest); System.out.println("Etag:" +
	 * result.getETag() + "-->" + result); } catch(Exception ex) {
	 * ex.printStackTrace(); } }
	 */
	...
	public void mongodb_v2(DatabaseManager dm) {
		 ...
		 * System.out.println(MyUtils.convertDatetoUnixTimeStamp(
		 ...
		 * 
		 * java.util.Date dt =
		 * MyUtils.convertUnixTimeStampToDateExample(Double.valueOf(val));
		 * 
		 * System.out.println(MyUtils.convertUnixTimeStampToDateExample(Double.
		 * valueOf(val))); System.out.println(getCurrentTimeStamp(dt));
		 * 
	...
	public static void readJSONFromURL() {
		...
		try {
			ObjectMapper mapper = new ObjectMapper();
			String json = readUrl(
					"http://cms.lifereimagined.org/webservice/getcontent.json?content_type=challenge_category");
			System.out.println(json);
	...
	public static void main(String[] args) throws MalformedURLException, IOException {
		 ...
		 * DatabaseManager(ConfigurationManager.jdbc_driver,
		 * ConfigurationManager.db_url, ConfigurationManager.database,
		 * ConfigurationManager.dbuser, ConfigurationManager.dbpassword,
		 * ConfigurationManager.getSince(), ConfigurationManager.batchsize, "");
		 */
		...
		// String json =
		// readJSONFromURL("https://cms.lifereimagined.org/webservice/getcontent.json?content_type=challenge_category");
		// System.out.println(json);

		/*
		 * String l_parentString =
		 * StringUtils.strip("results.unu.doi.trei.","\\.");
		 * System.out.println("l_parentString="+l_parentString); String[] parts
		 * = l_parentString.split("\\.");
		 * 
		 * 
		 ...
		 * 
		 * String aaa = ""; if (parts.length > 0) { for (int
		 * i=1;i<parts.length;i++) { aaa += parts[i] + "."; } }
		 * 
		...
		if (adminCheck) {
			if (s.indexOf("successAllAdmin") > -1 || s.indexOf("successIRDBAdmin") > -1
					|| s.indexOf("successIRDBSuper") > -1)
