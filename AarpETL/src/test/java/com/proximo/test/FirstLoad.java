	...
	public static void process(String srcJsonFile) {
		...
		try {
			...
			if (!dm.getmETLStillRun()) {
				...
				// build API URL
				// url =
				// builJsonURL(String.valueOf(since),String.valueOf(page),ConfigurationManager.jsonlimit,ConfigurationManager.jsonkey);
				...
				// debug
				System.out.println("page=" + page + ";item.size=" + dm.getProcessing().getJsonArray().size()
						+ ";gettime=" + (etEnd - etStart));
				page++;
				totalEt += (etEnd - etStart) / 1000;
				...
				// end the loop
				// if (!etlFlag || (dm.getProcessing().getJsonArray().size() <
				// ConfigurationManager.getLimit()) ||
				// (ConfigurationManager.batchloop != 0 && page >
				// ConfigurationManager.batchloop)) {
				// if (!etlFlag || page > 5) {
				// break;
				// }
				// }
