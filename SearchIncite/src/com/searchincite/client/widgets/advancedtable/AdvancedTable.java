...
public class AdvancedTable extends Composite {
	...
	public AdvancedTable() {
		...
		navigationPanel.setCellHorizontalAlignment(buttonPrevPage, HasHorizontalAlignment.ALIGN_RIGHT);
		...
		navigationPanel.setCellHorizontalAlignment(buttonNextPage, HasHorizontalAlignment.ALIGN_RIGHT);
		...
		navigationPanel.setCellHorizontalAlignment(buttonLastPage, HasHorizontalAlignment.ALIGN_RIGHT);
	...
	/**
	 * Shows or hides the first table column. The value in the first column is
	 * considered a row identifier (primary key). It can be visible or hidden
	 * from the user.
	 */
	...
	/**
	 * Sets a table model for this table, updates its columns and rows based on
	...
	/**
	 * Updates and redraws the table data rows of the currently selected page
	 * based on the information from the table model coming from the server.
	 * This method should be called after each change of the data at the server
	 * side (e.g. when applying data filtering or need to refresh the table
	 * rows).
	 */
	...
	private int calcPagesCount() {
		int pagesCount = (this.totalRowsCount + pageSize - 1) / this.pageSize;
		return pagesCount;
	}
	...
	private void redrawCheckBoxes() {
		for (int row = 0; row < this.pageSize; row++) {
			if (row < this.currentPageRowsCount) {
				...
				grid.getCellFormatter().setHorizontalAlignment(row + 1, 0,
	...
	/**
	 * @return the row identifier (primary key) of the currently selected row.
	 *         This is the value of the first column in the table. If no row is
	 *         currently selected, the returned value is null.
	 */
	 ...
	 *         Row identifiers are strings (primary key) that uniquely
	 *         identifies a row.
	 */
	...
	/**
	 * Marks all rows in the table matching the current filters without clearing
	 * the currently marked rows. Attention: This operation causes all data rows
	 * to be retrieved from the server side and this could be slow.
	 */
	...
	private void fixGridSize() {
		...
		String originalWidth = DOM
				.getStyleAttribute(this.getElement(), "width");
		scrollPanelGrid.setWidth(originalWidth);

		// Fix scroll panel height
		String originalHeightStr = DOM.getStyleAttribute(this.getElement(),
				"height");
		if (originalHeightStr.endsWith("%")) {
			...
			originalHeightStr = DOM.getElementProperty(parentContainer,
					"offsetHeight");
			...
			originalHeightStr = originalHeightStr.substring(0,
					originalHeightStr.length() - 2);
		}
