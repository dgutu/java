...
public class PrintLayoutPDF {
	...
	public PrintLayoutPDF() {
	}
	
	public static byte[] generatePDF(DOTradeName tn, String text, String basepath, String appUrl) throws DocumentException, MalformedURLException, IOException {
		...
		addCopyright(pdfDocument, appUrl);
		pdfDocument.close();
	...
	private static void addBodyText(Document document, String text) throws DocumentException {
		...
		if (text.toUpperCase().contains(inciEmail.toUpperCase())) {
			String first = text.substring(0, text.indexOf(inciEmail));
	...
	private static void addCopyright(Document document, String appUrl) throws DocumentException, MalformedURLException {		
	    ...
	    policy.setAction(new PdfAction(new URL(appUrl + "VAADIN/themes/inci-app-theme/privacypolicy.html")));
		document.add(policy);
