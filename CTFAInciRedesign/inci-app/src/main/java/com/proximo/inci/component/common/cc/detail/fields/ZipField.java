...
public class ZipField extends AbstractInciFieldComponent<TextField> {

    private AbstractCreditCardDetailLayout creditCardDetailLayout;

    public ZipField(AbstractCreditCardDetailLayout creditCardDetailLayout) {
    ...
    protected void init() {
        ...
        mainComponent.setCaption(CaptionResolver.getCaption("credit_card.detail.zip"));
    }

    @Override
    public void generateContent() {
        DOUserCC userCC = creditCardDetailLayout.getUserCC();
        if (userCC != null && !TemplarUtil.isStringNullOrEmpty(userCC.zip)) {
            setValue(userCC.zip);
        } else {
            setValue("");
        }
    ...
    public String getFieldDescription() {
        return CaptionResolver.getCaption("credit_card.detail.zip.DESC");
    }
